...
class YouTubeBulk {
  ...
  async run(source, opts = {}) {
    ...
    const existingFm = existingPlaylist
      ? this.plugin.app.metadataCache.getFileCache(existingPlaylist)?.frontmatter ?? {}
      : {};
    // dl-all is evidence, not a claim: true only when every video note in
    // this playlist has its media on disk right now. A note created a moment
    // ago has no media, so a sync with new videos always comes out false.
    let allOnDisk = created === 0 && noteFiles.size > 0;
    if (allOnDisk) {
      for (const f of noteFiles.values()) {
        const fm = this.plugin.app.metadataCache.getFileCache(f)?.frontmatter ?? {};
        if (this.plugin.mediaOnDisk(fm, f.path).length === 0) { allOnDisk = false; break; }
      }
    }
    const body = this.buildPlaylistNote(result, written, existingFm, allOnDisk);
  ...
  // The note is rebuilt on every sync, so the properties it already carries
  // come across -- anything a person added, which a sync cannot reconstruct.
  // 'dl-all' is not carried: the caller has just checked the disk.
  buildPlaylistNote(result, noteLinks, existingFm = {}, allOnDisk = false) {
    ...
    const wanted = (this.settings.playlistTags || []).map(hashTag).filter(Boolean);
    const had = [].concat(existingFm.tags ?? []).map(hashTag).filter(Boolean);
    const fm = {};
    for (const [k, v] of Object.entries(existingFm)) {
      if (k !== 'position') fm[k] = v;
    }
    Object.assign(fm, {
      'dl-all': allOnDisk === true,
      count: noteLinks.length,
      url: `[Link](${result.url})`,
      tags: [...had, ...wanted.filter((t) => !had.includes(t))],
    });
    const body = [
