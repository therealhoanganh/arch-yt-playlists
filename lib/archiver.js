...
class YouTubeBulk {
  ...
  async run(source, opts = {}) {
    ...
    const existingFm = existingPlaylist
      ...
      : {};
    const body = this.buildPlaylistNote(result, written, existingFm, created);
  ...
  // come across: 'dl-all' as it was, unless this sync created a video note --
  // then it is false again until the new one is downloaded -- and anything a
  // person added, which a sync cannot reconstruct. Rebuilding from scratch
  // reset dl-all on every sync whether or not anything was new.
  buildPlaylistNote(result, noteLinks, existingFm = {}, created = 0) {
    ...
    Object.assign(fm, {
      'dl-all': created === 0 && existingFm['dl-all'] === true,
